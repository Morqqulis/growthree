				...
				if (item.link === '/') {
					return (
						<li className={`block w-full text-center md:w-auto`} key={item.id}>
							<Link
								className={`headerLink block w-full px-2 py-10 md:py-0 ${path === item.link ? 'text-white before:scale-100 after:scale-100' : 'before:scale-0 after:scale-0'}`}
					...
					return (
						<li className={`block w-full text-center md:w-auto`} key={item.id}>
							<span
								className={`headerLink block w-full whitespace-nowrap py-10 md:py-0 ${path === item.link ? 'text-white before:scale-100 after:scale-100' : 'before:scale-0 after:scale-0'}`}>
								{item.title}
							</span>
						</li>
					)
